...
class ADMMSolver:
    ...
    def CheckConvergence(self, z_old, e_abs, e_rel, verbose=False):
    ...
    #solve
    def __call__(self, maxIters, eps_abs, eps_rel, verbose=False):
        num_iterations = 0
        ...
        for i in range(maxIters):
            ...
            self.ADMM_u()
            if i != 0:
                stop, res_pri, e_pri, res_dual, e_dual = self.CheckConvergence(z_old, eps_abs, eps_rel, verbose=False)
                if stop:
                    self.status = 'Optimal'
                    break
                new_rho = self.rho
